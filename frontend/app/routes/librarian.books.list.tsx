...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  return await makeAuthenticatedRequest(request, async () => {
    ...
    const totalBooks = books.reduce((sum: number, book: Book) => sum + book.total_count, 0);
    const totalAvailable = books.reduce((sum: number, book: Book) => sum + book.available_count, 0);
          ...
          <Card>
            ...
            <CardContent>
              <ScrollArea className="h-[600px]">
                <div className="space-y-2">
                  {books.map((book) => (
                    <BookItem key={book.id} book={book} showCategory showLanguage />
                  ))}
                </div>
              </ScrollArea>
            </CardContent>
...
function BookItem({ 
  ...
  const availabilityColor = 
    availabilityRate === 0 ? "destructive" : 
    availabilityRate < 50 ? "secondary" : 
    "default";
