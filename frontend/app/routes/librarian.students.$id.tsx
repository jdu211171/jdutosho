...
const rentalColumns: ColumnDef<RentalHistory>[] = [
	{
		...
		header: 'Code',
		cell: ({ row }) => (
			<Badge variant='outline'>{row.getValue('book_code')}</Badge>
		),
	},
	{
	...
	},
	{
		...
		cell: ({ row }) =>
			format(new Date(row.getValue('taken_at')), 'MMM d, yyyy'),
	},
	{
...
export default function StudentProfilePage() {
	const { student, activeRentals, rentalHistory, stats } =
					...
					<Tabs defaultValue='active'>
						...
						<TabsContent value='active' className='mt-4'>
							{activeRentals.length > 0 ? (
								<DataTable columns={rentalColumns} data={activeRentals} />
							) : (
						...
						<TabsContent value='history' className='mt-4'>
							{rentalHistory.length > 0 ? (
								<DataTable columns={rentalColumns} data={rentalHistory} />
							) : (
								<div className='text-center py-8 text-muted-foreground'>
									No rental history
								</div>
